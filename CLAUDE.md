...
6. **Polish** — layout save/load, per-tile options, proxy stream toggle, source name overlays, About dialog with NDI compliance items.
   ...
   - Max (2026-07-06): **native Companion module** — build a proper `companion-module-cinertia-mosaic` (Node.js/TypeScript on the Bitfocus module framework) with dropdown actions fed by the live profile list, feedbacks highlighting the active profile on Stream Deck buttons, variables (current profile/mode), and presets. The Mosaic-side protocol (PROFILES?/STATUS? queries + EVENT pushes) is already in place; distribute sideloaded first, submit to Bitfocus once the product is public.
